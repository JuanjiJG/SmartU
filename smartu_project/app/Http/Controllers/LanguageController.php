...
class LanguageController extends Controller
{
     ...
     * @param  \Illuminate\Support\Facades\Lang  $lang
     * @return redirect
     */
    public function switchLang(Request $request, $lang)
    {
        ...
        if (array_key_exists($lang, config('app.locales'))) {
            ...
            if ($route_name && Lang::has('routes.' . $route_name, $lang)) {
                ...
                if (count($query)) {
                    return redirect()->to($lang . '/' .  __('routes.' . $route_name, [], $lang) . '?' . http_build_query($query));
                }

                return redirect()->to($lang . '/' .  __('routes.' . $route_name, [], $lang));
            }
